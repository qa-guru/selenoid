...
+
```
$ docker run -it --rm --entrypoint /usr/bin/ffmpeg qaguru/video-recorder:latest -codecs
```
...
=== Saving Session HAR: enableHAR, harName, harContent

NOTE: This feature requires some preparation. Please refer to <<Saving Session HAR (Network Archive)>> section for more details.

To record a network archive (HAR) for a session, add:
...
The hub captures the browser network activity over CDP and writes a HAR file at
session close. This works for Chromium-based WebDriver sessions (Chrome / Edge)
in Docker mode and requires the `-har-output-dir` flag to be set.

By default saved HAR files are named `<session-id>.har` where `<session-id>` is a
unique identifier of the Selenium session. To provide a custom file name specify:

.Type: string
----
harName: "my-cool-session.har"
----

WARNING: It is important to add the `har` file extension.

Content depth (only meaningful with `enableHAR=true`; otherwise ignored):

.Type: string (`meta` | `bodies`, default `meta`)
----
harContent: "meta"
----

* `meta` (default) — headers/status/size/mimeType; no `content.text`.
* `bodies` — opt-in best-effort response bodies via CDP `Network.getResponseBody`
  (`content.text`, and `encoding: base64` when needed). Not equivalent to Playwright
  `recordHar`.

TIP: This is a hub artifact for manual/debugging sessions. For automated tests
prefer the client-side HAR recorder (WebDriver CDP → Allure, or Playwright
`recordHar`) and keep a single HAR writer per session — do not combine hub
`enableHAR` with client `recordHar` / `HarCapture` on the same session.
