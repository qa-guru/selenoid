...
IMPORTANT: AsciiDoc below is largely **upstream Aerokube** reference. For the qa-guru fork (native Playwright, `qaguru/*` images, Docker API `1.55`) start with the repository https://github.com/qa-guru/selenoid[README] and https://github.com/qa-guru/selenoid/blob/main/docs/browser-versions.md[browser-versions.md]. Docker image: `qaguru/selenoid` (not `aerokube/selenoid`).
...
It is using Docker to launch browsers. Please refer to https://github.com/qa-guru/selenoid[qa-guru/selenoid] (fork) or https://github.com/aerokube/selenoid[aerokube/selenoid] (upstream) for source code.
...
include::har.adoc[leveloffset=+1]
