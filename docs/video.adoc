...
To see available files use JSON listing (paginated, default page size 10 — never returns the full catalog in one response):

.Listing Video Files (JSON, page 1)
----
http://selenoid-host.example.com:4444/video/?json
----

.Response
----
{"videos":["<filename>.mp4"],"total":42,"limit":10,"offset":0}
----

Optional query params: `limit` (1..100, default 10), `offset` (default 0), `q` (substring filter on file name).

HTML directory listing (without `?json`) still works for manual browsing:
----
