...
-har-output-dir string
    Directory to save session HAR (HTTP Archive) files to
-limit int
...
-video-recorder-image string
    Image to use as video recorder (default "qaguru/video-recorder:latest")
-access-key string
    Comma-separated required ?accessKey= values for /playwright/ WebSocket (empty = no check)
-warm-pool-url string
    Warm-pool orchestrator base URL for /status warmReady/warmTotal/hotReady/hotTotal and Chrome WD attach (e.g. http://127.0.0.1:9090); empty disables
----
