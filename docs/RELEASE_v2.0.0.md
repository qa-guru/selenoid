...
3. Подтянуть образы:
   ...
   ```
4. Для Playwright за nginx — проксировать WebSocket `/playwright/` (см. `deploy/nginx-playwright-snippet.conf` в монорепо qa-guru).
...
| [qa-guru/selenoid_selenium_playwright](https://github.com/qa-guru/selenoid_selenium_playwright) | Примеры тестов (Java, Python, JS, TS) |
