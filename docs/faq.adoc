...
Then run Selenoid like the following:
    ...
    $ DOCKER_API_VERSION=1.55 ./selenoid <rest-of-args> # As a binary
    $ docker run -e DOCKER_API_VERSION=1.55 <rest-of-args> qaguru/selenoid:some-version # As Docker container
...
Yes, this is possible. See the following https://github.com/qa-guru/selenoid/issues/795[Github issue] for possible solutions.
