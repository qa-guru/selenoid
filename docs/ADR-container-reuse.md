# ADR: Container-reuse (warm Chrome WebDriver via `-warm-pool-url`)
...
| Term | Pool | Means | Not |
|------|------|-------|-----|
| **container-reuse** | warm | New Session on a container that is already up | Allure attachment; existing WD/PW session |
...
Three named pools. This ADR covers **warm** container-reuse only. **Cold** is the default, already-live path (hub `docker run` from `browsers.json`). **Hot** (session-reuse / preopen / bypass hub) is backlog window 03 — not this ADR.

| Pool | Role |
|------|------|
| **Cold** | Default. Existing Docker starter. Unchanged. |
| **Warm** | This ADR: Chrome WD **container-reuse** on a pre-started loopback slot. |
| **Hot** | **session-reuse**: same session + live page, client bypasses the hub. Not this ADR. |
...
Jenkins on box2 already talks to the orchestrator (`POST /pool/reserve`) and drives slots directly. Hub-side container-reuse was the documented follow-up.
...
1. **Chrome WebDriver only.** `Find()` may reuse a reserved warm container instead of starting Docker. Playwright, Firefox, Edge, Android — unchanged (cold).
...
5. **Same `-warm-pool-url`.** No second flag. Empty URL keeps today’s behaviour (no probe, no container-reuse).
...
- Local: `config.local.yaml` publishes `127.0.0.1:14441` (headed `:149`) and `14442` (`:149-min`, shm 2g) plus PW WS `14501/14502` — container-reuse works when those ports listen.
- Box1 hub compose: `webdriver_url_loopback` + publish `127.0.0.1:14441→4444` (`qaguru/webdriver-chrome:149`, shm 2g) and `14442` (`:149-min`, shm 2g). Container-reuse live on [selenoid.qa.guru](https://selenoid.qa.guru) for Chrome WD without video/VNC/HAR. Headed slot is first so the warm-pool Jenkins job prefers it.
...
**Playwright container-reuse stays out.** Warm 4/4 means four containers up (`webdriver-chrome:149`, `:149-min`, `playwright-chromium:1.61.1`, `:1.61.1-min`). Hub `Find()` reuse remains Chrome WD only (`warmEligible`). Playwright sessions still **cold**-start. PW slots are listed in the orchestrator (metrics + window 03 session-reuse over WS, bypassing the hub). A later ADR change for PW container-reuse would be a hub cut (`FindPlaywright` + loopback WS), not compose.

**`-min` WD:** shm **2g** (256m died on New Session on box1). No extra client headless caps — the `-min` image is already headless.

**Moved to window 03 (hot / session-reuse):** `PREOPEN_URL`, `WarmRemote`, skip `open()`, JVM-keep / `closeAfterAll` off the wall, frontend refresh on GitHub deploy. Not this ADR.
