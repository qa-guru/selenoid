...
EXPECTED_GO_MIN="1.23"
EXPECTED_DOCKER_API="1.45"
EXPECTED_DOCKER_ENGINE_PREFIX="26.1"

fail=0

echo "==> Go (minimum ${EXPECTED_GO_MIN}.x, CI uses ${EXPECTED_GO_MIN}.8)"
if command -v go >/dev/null 2>&1; then
  ...
  echo "    WARN: go not in PATH (build scripts use golang:1.23)" >&2
  fail=1
fi

echo "==> Docker Engine (recommended ${EXPECTED_DOCKER_ENGINE_PREFIX}.x, API ${EXPECTED_DOCKER_API})"
